...
function FI_Click(slot, button)
    if CursorHasItem() then
        ...
        -- Scan all bags for locked slot (cursor source)
        for bag = 0, 4 do
            local slots = GetContainerNumSlots(bag)
            if slots then
                for slotIndex = 1, slots do
                    local texture, itemCount, locked = GetContainerItemInfo(bag, slotIndex)
                    if locked then
        ...
        if itemLink then
            -- Extract itemID using string.find (Vanilla safe)
            local _, _, id = string.find(itemLink, "item:(%d+)")
            local itemID = tonumber(id)

            if itemID then
                FI_Config["Options"][slot] = itemID
                FI_SetSlot(slot)
            end
        end
